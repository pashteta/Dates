...
NSFetchedResultsControllerDelegate{
    ...
    @IBAction func behaviourNotification(_ sender: Any) {

        var fetchResultsController: NSFetchedResultsController<Decision>!
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext

        let ac = UIAlertController(title: "Выберите статус", message:  " Хотите ли вы получать уведомления ?", preferredStyle: .actionSheet)
        let yesAction = UIAlertAction(title: "Yes", style: .default) { (values) in

        }

        let noAction = UIAlertAction(title: "No", style: .default) { (values) in
